...
class LeafletMapLegend extends Component {
  render() {
		...
		let { alias_name, min, max, label, legend } = this.props.mapLayers["metric"];
		// console.log(this.props.mapLayers["metric"])
		let about_text = _util.getDescription(label)
		...
		return ( <div>
			...
			<Paper className={classes.legendContainer}>
					<div >
						<div className={classes.titleText}>{ alias_name }</div>
